...
/// Singleton pattern is used to load widget data only once.
class Meta {
  dynamic _config;
  ...
  factory Meta() {
    if (instance == null) {
      ...
      // generated by mdtest.
      dynamic widget = JSON.decode(tempFile.readAsStringSync());
      instance = new Meta._internal(widget);
    }
    return instance;
  }

  dynamic get widget => instance._config;
...
class DriverMap extends UnmodifiableMapBase<String, Future<FlutterDriver>> {
  ...
  Future<FlutterDriver> operator [](String nickname) async {
    dynamic widget = _meta.widget;
    if (widget.containsKey(nickname)) {
      if (!_map.containsKey(nickname)) {
        String observatoryUrl = widget[nickname]['observatory-url'];
  ...
  @override
  Iterable<String> get keys => _meta.widget.keys;
